    ...
    return (
        <div>
            ...
            {!isConnected && (
                <button onClick={() => {
                    setModal(true)
                }} className={`hitespace-nowrap px-5 py-2 border border-transparent rounded-lg shadow-sm text-base font-medium text-white bg-blue-600 hover:bg-blue-700`}>
                    Login Now
                </button>
            )}
            ...
            {isConnected && (
                <Link href="/account" className="w-[150px]">
                    <User size={24} className={`my-auto mx-auto cursor-pointer mb-0.5 ${path === "/account" ? "text-blue-400" : "text-white hover:text-blue-400"} `} />
                </Link>
            )}
        </div>
    )
